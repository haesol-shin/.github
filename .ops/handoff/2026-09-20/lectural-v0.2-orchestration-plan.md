...
First complete and release `repo-ops/v0.1.0` in `haesol-shin/.github`. Bootstrap PR #4 is merged. Revise and re-approve the Issue #3 plan, complete its feature candidate for changelog lifecycle, raw schema identifiers, release policy, and language-independent conformance fixtures, then execute high-risk Issue #5 in a dedicated OMP session. Issue #5 adds the Go evaluator and collector, proves zero-divergence compatibility and preserved trust boundaries, records performance honestly, defines a trusted production packaging path, and performs one clean Python-to-Go cutover. Prepare the separate Issue #3 release pull request, tag protection, and explicit release approval only after that decision. After the immutable central release exists, open a dedicated high-risk LecturAL adoption Issue in another OMP session.
