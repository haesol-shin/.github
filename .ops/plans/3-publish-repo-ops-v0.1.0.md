...
The trusted workflow separates contract validity from merge authorization. `CI / quality`, `Repository policy / contract`, and `Repository policy / merge approval` are the stable required-check targets; applying them to a repository ruleset remains a separate approval. The contract check runs in enforce mode from immutable base-owned code. The merge-approval check always runs and fails closed unless contract validation succeeds and the exact head has an authorized merge-review record. Pull request, review, and candidate approval-comment changes or deletions revoke stale authorization and re-publish both results on the pull request head. One evaluator owns the rules; workflows do not duplicate its policy logic.
