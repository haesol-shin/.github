...
The feature candidate uses `Related #3`, declares `Changelog: required`, adds `changelog.d/3-repo-ops-v0.1.0.md`, and does not edit `CHANGELOG.md`. After it merges, Issue #5 executes the separately authorized Go validator migration against the completed contract and language-independent conformance suite. If its compatibility, security, performance, and complexity gates pass, it performs one clean production cutover; otherwise production remains Python and the failed gate is recorded. Only after that decision does the folding command prepare a separate release pull request using `Related #3` and `Changelog: release — v0.1.0`, create the initial `CHANGELOG.md`, and delete the consumed fragment.

Bootstrap merge, feature merge, the Issue #5 cutover decision, release merge, tag protection, release approval, annotated tag creation, GitHub Release publication, and Issue closure remain separate gates. Before publication, the maintainer must approve the ruleset change protecting `v*` and later approve the exact verified release candidate. After publication, never move, delete, or recreate `v0.1.0`; correct defects with a new version or document a blocking limitation.
...
11. Execute Issue #5 in its dedicated OMP session. Add the fixture-only Go evaluator, differential conformance, live collector, sanitized event replay, shadow evidence, and benchmark record in separately reviewable pull requests. Cut production over once only if every approved gate passes; otherwise preserve the Python production path and record the failed criterion.
12. From verified `main` after the Issue #5 decision, run the folding command to prepare a release pull request using `Related #3` and `Changelog: release — v0.1.0`; create the dated `CHANGELOG.md` entry and delete only the consumed fragment.
...
After the feature merge, require green `CI / quality`, then complete the Issue #5 decision with zero conformance divergence, preserved trust boundaries, and recorded startup, clean-CI p50/p95, peak-memory, workflow-step, and dependency measurements. A Go cutover additionally requires at least 2x faster cold CLI startup, at least 20% lower clean-CI p50, no CI p95 or peak-memory regression, and no increase in bootstrap or dependency complexity. After the decision, use the shipped folding command for the real release pull request. Prove that the generated changelog matches fragment content, the correct fragment is deleted, no unrelated file is consumed, and the release declaration names `v0.1.0`. Require green post-merge CI on the exact release candidate.
...
If bootstrap review or its exception is rejected, do not merge it or start feature work. If planning, feature review, Issue #5 parity or measurement, release review, or CI fails, repair the applicable branch without tagging; a failed Go gate keeps Python production rather than creating a partial or permanent dual implementation. If release-candidate verification or tag protection fails, revert the release candidate and do not publish. After publication, preserve the immutable tag, state any defect as a limitation, block consumer adoption when necessary, and publish a corrective version rather than rewriting history.
