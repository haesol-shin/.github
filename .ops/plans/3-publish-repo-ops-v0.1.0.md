...
5. Prepare the feature pull request with `Related #3`, record approval naming the current reviewed plan commit as `plan-commit`, and extend `contract.json`, the pull request template, and fixtures with the exact changelog declaration and values.
